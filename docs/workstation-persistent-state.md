...
Copy `proxmox_lxc` and `proxmox_lxc.pub` back from your own backup. No playbook regenerates them; an SSH-consuming live command fails with a controller-identity diagnostic until the pair is in place, because a freshly minted key is one no LXC in the fleet trusts.
