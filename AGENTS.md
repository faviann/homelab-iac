...
| `./setup.sh bootstrap` | Eagerly reconcile every declared collection and external role, and create the controller SSH key when absent (never replaces an existing key). Live commands do not require it for worktree dependencies; a new controller may require it before managed-host operations |
...
**Live dependencies**: `./run.sh`, live `./inspect.sh`, and `./recover.sh` reconcile their own worktree dependencies. Once the shared boundary holds the lifecycle lock and before it invokes Ansible, it runs `scripts/live_dependencies.py`, which installs and verifies, by name and at its own pin, only the collections and external roles the selected playbook consumes, then creates `.ansible/cp/` for SSH-consuming operations. This does not create controller identity: managed-host operations still require `~/.ansible/ssh/proxmox_lxc`, which `./setup.sh bootstrap` creates when absent. Reconciling under the held lock keeps contention fail-fast: a caller that loses the lock exits 75 without downloading anything. That works because `LIVE_OPERATIONS` in that module records what each playbook reaches; it breaks when a playbook starts consuming something nobody added to its row, and the run then fails naming the unpinned dependency. The safer rule: change the registry row in the same commit that changes what a playbook consumes, and never widen reconciliation back to a whole declaration file — a dependency one operation does not consume must never block it.
