...
| `./validate.sh` | Complete non-live verification (~5 min) — lint, full lifecycle regressions, and the pytest suite except the real Renovate gate |
| `./validate.sh lifecycle` | Fast lifecycle feedback (~1 min) — semantic lifecycle facade matrix + targeted planning barrier, controlled observations only. Run while iterating on LXC lifecycle changes |
...
→ [stacks/broodling/broodling/README.md](stacks/broodling/broodling/README.md) — read before touching the `broodling` LXC, its stack, or its one-time initialization.
